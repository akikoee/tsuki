...
type Props = {
  ...
  events: { index: number; name: string; status: "matched" | "low-confidence" | "unmatched" }[];
};

export default React.memo(function ProgressArea(props: Props) {
  const CHEVRON_WIDTH = 420;
  const progressWidthPx = useMemo(() => Math.round((CHEVRON_WIDTH * props.progressPercent) / 100), [props.progressPercent]);
  const chevronRotation = useMemo(() => props.direction === "right" ? 0 : 180, [props.direction]);
        ...
        <>
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2">
            <div className="mb-2 h-5 text-center text-sm text-muted-foreground">
              {props.isTransferring ? `Transferring ${props.currentPlaylistName ?? "..."}` : ""}
            </div>
              ...
              >
                ...
                <g stroke="rgba(163,163,163,0.7)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none">
                  {Array.from({ length: 20 }).map((_, i) => {
                    const x = i * 21 + 6;
                    return <polyline key={i} points={`${x},6 ${x + 8},14 ${x},22`} />;
                  })}
                </g>
                <g clipPath="url(#chevClip)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none">
                  {Array.from({ length: 20 }).map((_, i) => {
                    ...
                    const color = rgbToHex(a.r + (b.r - a.r) * t, a.g + (b.g - a.g) * t, a.b + (b.b - a.b) * t);
                    return <polyline key={`p${i}`} points={`${x},6 ${x + 8},14 ${x},22`} stroke={color} />;
                  })}
                </g>
              </svg>
            </div>
          ...
          <div className="absolute inset-x-0 top-[calc(50%+44px)] h-64 overflow-y-auto">
            <ul className="space-y-1">
              {props.events.map((t) => (
                <li key={t.index} className="flex items-center justify-between text-sm animate-[flyin_.4s_ease-out]">
                  <span className="truncate pr-2 text-muted-foreground">{t.name}</span>
                  <span className={t.status === "matched" ? "text-emerald-500" : t.status === "low-confidence" ? "text-yellow-500" : "text-rose-500"}>
                    {t.status}
                  </span>
                </li>
              ))}
              {!props.events.length && <li className="text-sm text-muted-foreground">&nbsp;</li>}
            </ul>
          </div>
        </>
      )}
